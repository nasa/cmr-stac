...
async function getCollections(request, response) {
  ...
  const { currPage, prevResultsLink, nextResultsLink } =
    generateNavLinks(event);
  ...
  const collectionsResponse = {
    ...
    links: [
      wfs.createLink(
        "self",
        generateAppUrl(event, `/${provider}/collections`),
        `${description}`
      ),
      wfs.createLink("root", generateAppUrl(event, "/"), `${rootName}`),
    ],
    collections: collections.map((coll) =>
      convert.cmrCollToWFSColl(event, coll)
    ),
  };
...
async function getCollection(request, response) {
  logger.info(
    `GET /${request.params.providerId}/collections/${request.params.collectionId}`
  );
  const event = request.apiGateway.event;
  ...
  if (!collection) {
    throw new errors.NotFound(
      `Could not find collection with id [${collectionId}] for provider [${providerId}]`
    );
  }
...
async function getItems(request, response) {
  ...
  const cmrCollectionId = await cmr.stacIdToCmrCollectionId(
    providerId,
    stacCollectionId
  );
  ...
  if (stacCollectionId) {
    if (!cmrCollectionId) {
      return response
        ...
        .json({
          errors: [
            `Collection [${stacCollectionId}] not found for provider [${providerId}]`,
          ],
        });
    }
    ...
    if (params.collections) {
      return response
        ...
        .json({
          errors: [
            `Can not have collections param when there is a collectionId [${stacCollectionId}] specified`,
          ],
        });
    }
  ...
  if ((collectionsRequested && validCollections) || !collectionsRequested) {
    ...
    if (settings.cmrStacRelativeRootUrl === "/cloudstac") {
      ...
      const allCloudCollections = await findCloudCollections(
        providerId,
  ...
  const parentPromises = granulesResult.granules
  ...
  const featureCollection = await convert.cmrGranulesToStac(
    event,
    parentCollectionMap,
...
async function getItem(request, response) {
  ...
  if (!cmrCollectionId) {
    return response
      ...
      .json({
        errors: [
          `Collection [${collectionId}] not found for provider [${providerId}]`,
        ],
      });
  }
  ...
  const parentCollection = await cmr.fetchConcept(
    granule.collection_concept_id
  );
  const granuleResponse = convert.cmrGranuleToStac(
    event,
    parentCollection,
    granule
  );
...
async function getCatalog(request, response) {
  ...
  if (day) {
    facets.itemids.forEach((id) =>
      cat.addItem(id, providerId, collectionId, id)
    );
  } else if (month) {
    facets.days.forEach((d) =>
      cat.addChild(`${year}-${month}-${d} catalog`, `/${d}`)
    );
  } else if (year) {
